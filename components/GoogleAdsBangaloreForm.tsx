...
const RECAPTCHA_ACTION = "google_ads_bangalore_lead";

export function GoogleAdsBangaloreForm() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch("/api/google-ads-bangalore-lead", {
        method: "POST",
        body: formData,
      });
  ...
  return (
    ...
    >
      ...
      <div>
        <label className="block text-sm font-medium text-slate-700">Monthly ad budget (optional)</label>
        <input
          type="text"
          name="adBudget"
          className="mt-1 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-900 outline-none ring-emerald-500/0 transition focus:border-emerald-500 focus:bg-white focus:ring-2 focus:ring-emerald-500/20"
          placeholder="e.g. ₹30,000 – ₹1,00,000"
        />
      </div>
